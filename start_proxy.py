    ...
    try:
        ...
        try:
            ...
            if (resp_verify.status_code == 200):
                ...
                logger.success(f"Mail verify (it's ok!): {mail} | Status code: {resp_verify.status_code} | Response text: {resp_verify.text}")
        except Exception as error:
            logger.error(f"Unexcepted error: {error}")
            return 1
        ...
        if ('access_token' in resp_verify.json()):
            ...
            try:
                ...
                if (resp_verify.status_code == 200):
                    ...
                    logger.error(f"Error mail verify (that is bad!): {mail} | Status code: {resp_verify.status_code} | Response text: {resp_verify.text}")
            except Exception as error:
                logger.error(f"Unexcepted error: {error}")
                return 1
